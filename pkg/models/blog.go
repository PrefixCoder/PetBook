...
func (b *BlogStore) GetBlogStats() (int, error) {
	rows, err := b.DB.Query("select blog_id from blog")
	if err != nil {
		return 0, fmt.Errorf("cannot connect to database: %v", err)
	}
	tRes := Blog{}
	var results []Blog
	for rows.Next() {
		...
		if err != nil {
			return 0, fmt.Errorf("cannot insert message to messages in db: %v", err)
		}
		tRes.Id = blogid
		results = append(results, tRes)
	}
