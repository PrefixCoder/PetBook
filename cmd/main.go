...
func main() {
	...
	authRouter.HandleFunc("/chats/{id}", controller.DeleteChatHandler()).Methods("POST") //does not work with method DELETE with overriding with js too
	authRouter.HandleFunc("/chats/{id}", controller.HandleChatConnectionGET()).Methods("GET")
	authRouter.HandleFunc("/chats/{id}/search/{date}", controller.HandleChatSearchConnection()).Methods("GET")
	authRouter.HandleFunc("/ws/{id}", controller.HandleChatConnection())
	...
	if err := http.ListenAndServe(":8080", basicRouter); err != nil {
